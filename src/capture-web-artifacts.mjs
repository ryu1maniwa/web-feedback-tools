...
const DEFAULT_OUTPUT_ROOT = resolve(WORKSPACE_ROOT, 'artifacts/web-feedback')
...
function printHelp() {
  console.log(`Usage: web-feedback-capture [options]
...
Shared options:
  ...
  --output-dir <path>      Output directory (default: artifacts/web-feedback/<timestamp>)
...
function selectViewports(requestedNames) {
  ...
  const selected = DEFAULT_VIEWPORTS.filter((viewport) => requestedNames.includes(viewport.name))
  if (selected.length === 0) {
    throw new Error(`No supported viewports selected: ${requestedNames.join(', ')}`)
  }
...
async function startManagedServer(serverCommand, cwd, readyUrl, baseUrl) {
  ...
  if (!probeUrl) {
    throw new Error('When using --server-command, provide --ready-url or --base-url')
  }
  ...
  return page.locator(sectionSelector).evaluateAll((elements) =>
    elements.map((element, index) => {
      ...
      const name = rawName
        ...
        .replace(/^-+|-+$/g, '')
      const heading =
        element.querySelector('h1, h2, h3')?.textContent?.replace(/\s+/g, ' ').trim() ?? rawName
...
async function captureViewportArtifacts(
  ...
  const sections =
    requestedSections.length === 0
      ? availableSections
      : availableSections.filter((section) => requestedSections.includes(section.name))
  ...
  for (const section of sections) {
    ...
    const sectionPath = sectionArtifactPath(outputDir, section.dirName, viewportConfig.name)
...
function resolveBaseUrl(options) {
  return options.baseUrl || options.url || (options.serverCommand ? '' : `http://${options.host}:${options.port}`)
}
...
async function main() {
  ...
  const outputDir = options.variant ? resolve(outputRoot, options.variant) : outputRoot
  ...
  if (!effectiveBaseUrl) {
    throw new Error('Provide --base-url/--url, or use --server-command, or rely on --project-dir mode')
  }
